"""
Perplexity API client implementation for LLM Answer Watcher.

Provides asynchronous HTTP client for Perplexity Chat Completions API with
...
- Supports Perplexity-specific features (Sonar models with web search)

Example:
    >>> from llm_runner.perplexity_client import PerplexityClient
    >>> client = PerplexityClient("sonar-pro", api_key="pplx-...",
...
class PerplexityClient:
    """
    Perplexity Chat Completions API client with async retry logic and cost tracking.

    Implements the LLMClient protocol for Perplexity's Chat Completions API with automatic retry
    ...
    Perplexity's API is OpenAI-compatible, making integration straightforward. The API supports
    both standard chat models and specialized Sonar models with web search capabilities.

    Attributes:
        model_name: Perplexity model identifier (e.g., "sonar-pro", "sonar", "sonar-reasoning")
        api_key: Perplexity API key for authentication (NEVER logged)
        system_prompt: System message sent with every request for context/instructions

    Example:
        >>> client = PerplexityClient("sonar-pro", "pplx-...", "You are a helpful assistant.")
    ...
    Note:
        ...
        Request fees (for Sonar models) are not yet included in cost estimates.
    """
    ...
    def __init__(
        ...
        """
        Initialize Perplexity client with model, API key, system prompt, and optional tools.

        Args:
            model_name: Perplexity model identifier (e.g., "sonar-pro", "sonar")
            api_key: Perplexity API key for authentication (starts with "pplx-")
        ...
        Note:
            Tools are not currently supported for Perplexity client in v1.
        ...
        if tools:
            logger.warning(
                f"Tools are not currently supported for Perplexity client. "
                f"Tools parameter will be ignored for model: {model_name}"
            )

        # Log initialization (never log api_key)
        logger.info(f"Initialized Perplexity client for model: {model_name}")
    ...
    async def generate_answer(self, prompt: str) -> LLMResponse:
        ...
        Sends prompt to Perplexity Chat Completions API with system message for
        ...
        Example:
            >>> client = PerplexityClient("sonar-pro", "pplx-...")
            >>> response = await client.generate_answer("What are the best CRM tools?")
        ...
        Note:
            ...
            Cost estimates include token costs but not request fees (Sonar models).
        """
        ...
        # Perplexity Chat Completions API uses OpenAI-compatible format
        payload = {
        ...
        logger.debug(f"Sending request to Perplexity: model={self.model_name}")
        ...
        try:
            async with httpx.AsyncClient(timeout=REQUEST_TIMEOUT) as client:
                ...
                if response.status_code in NO_RETRY_STATUS_CODES:
                    ...
                    raise RuntimeError(
                        f"Perplexity API error (non-retryable): "
            ...
            logger.error(
                f"Perplexity API connection error: model={self.model_name}, error={e}"
            )
            raise

        except httpx.TimeoutException as e:
            # Request timed out
            logger.error(f"Perplexity API timeout: model={self.model_name}, error={e}")
            raise
        ...
        try:
            ...
            raise RuntimeError(f"Failed to parse Perplexity response JSON: {e}") from e
        ...
        cost_usd = estimate_cost("perplexity", self.model_name, usage_meta)
        ...
        return LLMResponse(
            ...
            web_search_results=None,  # Web search results not extracted in v1
            web_search_count=0,
        )

    def _extract_answer_text(self, data: dict[str, Any]) -> str:
        """
        Extract answer text from Perplexity Chat Completions API response.

        Args:
            data: Parsed JSON response from Perplexity Chat Completions API
        ...
        """
        try:
            # Perplexity API uses OpenAI-compatible 'choices' array
            choices = data.get("choices")
            if not choices or not isinstance(choices, list) or len(choices) == 0:
                raise RuntimeError("Perplexity response missing 'choices' array")
            ...
            raise RuntimeError(f"Invalid Perplexity response structure: {e}") from e

    def _extract_token_usage(self, data: dict[str, Any]) -> tuple[int, int, int]:
        """
        Extract token usage breakdown from Perplexity Chat Completions API response.
        ...
        if not usage or not isinstance(usage, dict):
            logger.warning(
                f"Perplexity response missing 'usage' data for model={self.model_name}. "
                "Token count and cost will be zero."
            )
            return 0, 0, 0

        # Perplexity uses 'prompt_tokens' and 'completion_tokens' like OpenAI
    ...
    def _extract_error_detail(self, response: httpx.Response) -> str:
        """
        Extract error detail from Perplexity Chat Completions API error response.
