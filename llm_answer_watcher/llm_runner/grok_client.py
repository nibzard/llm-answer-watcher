...
# X.AI Grok API endpoint (OpenAI-compatible)
...
class GrokClient:
    ...
    Attributes:
        model_name: Grok model identifier (e.g., "grok-beta", "grok-2-1212")
        api_key: X.AI API key for authentication (NEVER logged)
    ...
    def __init__(
        ...
        """
        Initialize Grok client with model, API key, system prompt, and optional tools.

        Args:
            model_name: Grok model identifier (e.g., "grok-beta", "grok-2-1212")
            api_key: X.AI API key for authentication
        ...
        Note:
            Tools are not currently supported for Grok client in v1.
        ...
        if tools:
            logger.warning(
                f"Tools are not currently supported for Grok client. "
    ...
    async def generate_answer(self, prompt: str) -> LLMResponse:
        ...
        try:
            async with httpx.AsyncClient(timeout=REQUEST_TIMEOUT) as client:
                ...
                if response.status_code in NO_RETRY_STATUS_CODES:
                    ...
                    raise RuntimeError(
                        f"Grok API error (non-retryable): "
            ...
            logger.error(
                f"Grok API connection error: model={self.model_name}, error={e}"
            )
            raise

        except httpx.TimeoutException as e:
            # Request timed out
            logger.error(f"Grok API timeout: model={self.model_name}, error={e}")
            raise
        ...
        try:
            ...
            raise RuntimeError(f"Failed to parse Grok response JSON: {e}") from e
        ...
        cost_usd = estimate_cost("grok", self.model_name, usage_meta)
    ...
    def _extract_answer_text(self, data: dict[str, Any]) -> str:
        """
        Extract answer text from Grok Chat Completions API response.

        Uses OpenAI-compatible response format with 'choices' array.

        Args:
            data: Parsed JSON response from Grok Chat Completions API
        ...
        try:
            ...
            if not choices or not isinstance(choices, list) or len(choices) == 0:
                raise RuntimeError("Grok response missing 'choices' array")
            ...
            raise RuntimeError(f"Invalid Grok response structure: {e}") from e

    def _extract_token_usage(self, data: dict[str, Any]) -> tuple[int, int, int]:
        """
        Extract token usage breakdown from Grok Chat Completions API response.
        ...
        if not usage or not isinstance(usage, dict):
            logger.warning(
                f"Grok response missing 'usage' data for model={self.model_name}. "
                "Token count and cost will be zero."
            )
            return 0, 0, 0

        # OpenAI-compatible format uses 'prompt_tokens' and 'completion_tokens'
    ...
    def _extract_error_detail(self, response: httpx.Response) -> str:
        """
        Extract error detail from Grok Chat Completions API error response.
